...
if __name__ == '__main__':
    spotify_username = raw_input("Please give your Spotify username: ")
    #if len(sys.argv) > 1:
        #username = sys.argv[1]
    #else:
    ...
    if token:
        ...
        spotify_playlists = sp.user_playlists(spotify_username)
        for spotify_playlist in spotify_playlists['items']:
            if spotify_playlist['owner']['id'] == spotify_username:
                print()
                ...
                results = sp.user_playlist(spotify_username, spotify_playlist['id'], fields="tracks,next")
